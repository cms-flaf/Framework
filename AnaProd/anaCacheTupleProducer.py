...
def createAnaCacheTuple(anaDir, outDir, unc_cfg_dict, snapshotOptions, compute_unc_variations):
    ...
    #    os.remove(outDir)
    snaps = []
    input_files = [ f for f in os.listdir(anaDir) ]
    for inFile in input_files:
        inFileName = os.path.join(anaDir, inFile)
        outFileName = os.path.join(outDir, inFile)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--anaDir', required=True, type=str)
    ...
    print(args.outDir)
    createAnaCacheTuple(args.anaDir, args.outDir, unc_cfg_dict, snapshotOptions, args.compute_unc_variations)
