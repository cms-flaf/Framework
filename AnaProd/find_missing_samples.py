...
if __name__ == "__main__":
    ...
    samples_to_drop = []
    missing_samples = []
    samples_available = os.listdir(f'/eos/cms/store/group/phys_higgs/HLepRare/HTT_skim_v1/Run2_{args.year}')
    #print(samples_available)
    for sample,subdict in config.items():
        if sample=="GLOBAL": continue
        if sample in samples_available: continue
        samples_to_drop.append(sample)
    for sample_av in samples_available:
        if sample_av in config.keys(): continue
        if 'NMSSM' in sample_av: continue
        if 'UncorrelatedDecay' in sample_av: continue
