...
def GetBinValues(histogram):
    mean, sigma, integral = (
        histogram.GetMean(),
        histogram.GetStdDev(),
        histogram.Integral(0, histogram.GetNbinsX()),
    )
    ...
    for x_bin in range(0, histogram.GetNbinsX()):
        ...
        bin_center_prev = (
            histogram.GetXaxis().GetBinCenter(x_bin - 1) if x_bin > 0 else 0
        )
...
def getSignificance(hist_signal, hist_bckg):
    ...
    for x_bin in range(0, hist_bckg.GetNbinsX() + 1):
        ...
        if bin_value != 0.0 and (bin_error / bin_value) < 0.2:
    ...
    for bin_to_consider in bins_to_consider:
        s2 += hist_signal.GetBinContent(bin_to_consider) * hist_signal.GetBinContent(
            bin_to_consider
        )
        b += hist_bckg.GetBinContent(bin_to_consider)
        sigma2 += s2 / b if b != 0.0 else 0.0
...
if __name__ == "__main__":
    ...
    all_samples_list, all_samples_types = GetSamplesStuff(
        bckg_cfg_dict.keys(),
        sample_cfg_dict,
        os.path.join(args.histDir, f"v9_deepTau2p1"),
        False,
    )
    for deepTauVersion in ["2p1", "2p5"]:
        inDir = os.path.join(
            args.histDir,
            f"v9_deepTau{deepTauVersion}",
            "all_histograms",
            args.var,
            btag_dir,
        )
        outDir = os.path.join(
            args.outDir, f"v9_deepTau{deepTauVersion}", "s_sqrt_b", args.var, btag_dir
        )
        if not os.path.exists(outDir):
            os.makedirs(outDir)
        TwoDTrue = args.want2D and not args.wantBTag
        inFileName_prefix = (
            f"all_histograms_{args.var}2D" if TwoDTrue else f"all_histograms_{args.var}"
        )
        ...
        for signal in ["GluGluToRadion", "GluGluToBulkGraviton"]:
            ...
            for channel in ["eTau", "muTau", "tauTau"]:
                ...
                for category in ["inclusive", "res2b", "res1b"]:
                    hist_bckg_total = GetHisto(
                        channel, category, inFile, "QCD", "Central", "-"
                    )
                    ...
                    for sample in all_samples_list:
                        sample_type = (
                            sample
                            if not sample in sample_cfg_dict.keys()
                            else sample_cfg_dict[sample]["sampleType"]
                        )
                        # print(signals)
                        if sample_type in signals or sample_type == "data":
                            # print(f'ignoring {sample_type}')
                            continue
                        hists_to_merge.append(
                            GetHisto(channel, category, inFile, sample, "Central", "-")
                        )
                        integral_central = 0
                    hist_bckg_total.Merge(hists_to_merge)
                    for mass in all_masses:
                        hist_signal = GetHisto(
                            channel,
                            category,
                            inFile,
                            f"{signal}ToHHTo2B2Tau_M-{mass}",
                            "Central",
                            "-",
                        )
                        ...
                        (
                            ...
                            s,
                            sqrtb,
                        ) = getSignificance(hist_signal, hist_bckg_total)
                        mass_dict[signal][channel][category][mass][
                            "bins_to_consider"
                        ] = bins_to_consider
                        mass_dict[signal][channel][category][mass][
    ...
    outDir_2p1 = os.path.join(
        args.outDir, f"v9_deepTau2p1", "s_sqrt_b", args.var, btag_dir
    )
    outDir_2p5 = os.path.join(
        args.outDir, f"v9_deepTau2p5", "s_sqrt_b", args.var, btag_dir
    )
    ...
    print("\hline \\\\[-0.9em]")
    print(
        f"""signal & mass & category & percentage diff & version & $s/\sqrt{{b}}$ & s& $\sqrt{{b}}$ \\\\"""
    )  # & bins & bins values & bins centers & bins errors  \\\\""")
    print("\hline")
    print("\hline")
    k = 0
    j = 0
        ...
        for channel in ["eTau", "muTau", "tauTau"]:
            for category in ["inclusive", "res2b", "res1b"]:
                ...
                for mass in all_masses:
                    j += 1
                    ratio_2p5_2p1 = (
                        dict_2p5[channel][category][f"{mass}"]["sigma"]
                        / dict_2p1[channel][category][f"{mass}"]["sigma"]
                        if dict_2p1[channel][category][f"{mass}"]["sigma"] != 0.0
                        else dict_2p5[channel][category][f"{mass}"]["sigma"]
                    )
                    if ratio_2p5_2p1 != 0:
                        all_ratios_2p5_2p1.append(ratio_2p5_2p1)
                        all_masses_ratios_hist.append(mass)
                    ssqrtb_2p1 = round(
                        dict_2p1[channel][category][f"{mass}"]["sigma"], 2
                    )
                    ssqrtb_2p5 = round(
                        dict_2p5[channel][category][f"{mass}"]["sigma"], 2
                    )

                    if ssqrtb_2p1 != 0 or ssqrtb_2p5 != 0:
                        all_masses_hist.append(mass)
                        all_sqrts_2p1.append(
                            dict_2p1[channel][category][f"{mass}"]["sigma"]
                        )
                        all_sqrts_2p5.append(
                            dict_2p5[channel][category][f"{mass}"]["sigma"]
                        )
                    ...
                    sqrtb_2p1 = round(
                        dict_2p1[channel][category][f"{mass}"]["sqrtb"], 2
                    )
                    sqrtb_2p5 = round(
                        dict_2p5[channel][category][f"{mass}"]["sqrtb"], 2
                    )

                    dict_mass_2p5 = dict_2p5[channel][category][f"{mass}"]
                    dict_mass_2p1 = dict_2p1[channel][category][f"{mass}"]

                    bins_centers_2p1 = dict_2p1[channel][category][f"{mass}"][
                        "bins_centers"
                    ]
                    bins_centers_2p5 = dict_2p5[channel][category][f"{mass}"][
                        "bins_centers"
                    ]
                    bins_values_2p1 = dict_2p1[channel][category][f"{mass}"][
                        "bins_values"
                    ]
                    bins_values_2p5 = dict_2p5[channel][category][f"{mass}"][
                        "bins_values"
                    ]
                    bins_errors_2p1 = dict_2p1[channel][category][f"{mass}"][
                        "bins_errors"
                    ]
                    bins_errors_2p5 = dict_2p5[channel][category][f"{mass}"][
                        "bins_errors"
                    ]
                    bins_to_consider_2p1 = dict_2p1[channel][category][f"{mass}"][
                        "bins_to_consider"
                    ]
                    bins_to_consider_2p5 = dict_2p5[channel][category][f"{mass}"][
                        "bins_to_consider"
                    ]
                    if ssqrtb_2p5 != 0.0 and (ssqrtb_2p1 > ssqrtb_2p5):
                        percentage_diff = round(
                            100 * (ssqrtb_2p1 - ssqrtb_2p5) / (ssqrtb_2p1 + ssqrtb_2p5),
                            2,
                        )
                        if percentage_diff > 5:
                            ...
                            print(
                                f"""2p1 & {ssqrtb_2p1} & {s_2p1} & {sqrtb_2p1} \\\\"""
                            )
                            print(
                                f"""& & 2p5 & {ssqrtb_2p5} & {s_2p5} & {sqrtb_2p5} \\\\"""
                            )
                            print("\hline")
                ...
                # plt.ylim(-1.5, 2.0)
                plt.savefig(
                    f"{plt_ratios_Dir}/{channel}_{category}_{signal}_S_ratios.png"
                )
                plt.clf()
