...
def GetShapeDataFrameDict(all_dataframes, global_cfg_dict, key, key_central, inFile, inFileCache, compute_variations, period, deepTauVersion, colNames, colTypes, region, isData, hasCache=True ):
    ...
    if compute_variations and key!=key_central and sample_type!='data':
        ...
        if treeName_noDiff in file_keys:
            ...
            dfWrapped_noDiff = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_noDiff, inFile),global_cfg_dict, period, deepTauVersion=deepTauVersion, region=region,isData=isData)
            dfWrapped_noDiff.CreateFromDelta(colNames, colTypes)
            if hasCache:
                dfWrapped_cache_noDiff = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_noDiff,inFileCache),global_cfg_dict, period,  deepTauVersion=deepTauVersion, region=region,isData=isData)
        ...
        if treeName_Valid in file_keys:
            ...
            dfWrapped_Valid = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_Valid, inFile),global_cfg_dict, period,  deepTauVersion=deepTauVersion, region=region,isData=isData)
            dfWrapped_Valid.CreateFromDelta(colNames, colTypes)
            if hasCache:
                dfWrapped_cache_Valid = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_Valid,inFileCache), global_cfg_dict,period,  deepTauVersion=deepTauVersion, region=region,isData=isData)
        ...
        if treeName_nonValid in file_keys:
            ...
            dfWrapped_nonValid = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_nonValid, inFile),global_cfg_dict, period,  deepTauVersion=deepTauVersion, region=region,isData=isData)
            if hasCache:
                dfWrapped_cache_nonValid = DataFrameBuilderForHistograms(ROOT.RDataFrame(treeName_nonValid,inFileCache), global_cfg_dict, period, deepTauVersion, args.region,isData)
...
if __name__ == "__main__":
    ...
    if not create_new_hist:
        ...
        if hasCache:
            dfWrapped_cache_central = DataFrameBuilderForHistograms(ROOT.RDataFrame('Events',args.cacheFile),global_cfg_dict, args.period, deepTauVersion=args.deepTauVersion, region=args.region,isData=isData)
        ...
        if args.compute_unc_variations and args.dataset!='data':
            for uncName in unc_cfg_dict['shape']:
                ...
                for scale in scales:
                    key_2 = (args.sampleType, uncName, scale)
                    print(key_2)
                    GetShapeDataFrameDict(all_dataframes, global_cfg_dict, key_2, key_central, args.inFile, args.cacheFile, compute_variations, args.period, args.deepTauVersion, col_names_central, col_tpyes_central, args.region, isData, hasCache)
