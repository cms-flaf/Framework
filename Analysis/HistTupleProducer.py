...
def createHistTuple(inFile, cacheFile, treeName, outFile, setup, hist_cfg_dict,unc_cfg_dict, snapshotOptions,range, evtIds,  compute_unc_variations, compute_rel_weights, histTupleDef):
    ...
    for unc in all_rel_uncs_to_compute:
        ...
        for scale in scales:
            ...
            dfw_unc_scale = histTupleDef.GetDfw(df_unc_scale, df_unc_scale_cache, setup.global_params)
            final_weight_name = "weight_for_hists"
            histTupleDef.DefineWeightForHistograms(dfw_unc_scale, unc, scale, sample_type, unc_cfg_dict, hist_cfg_dict, setup.global_params, final_weight_name)
            dfw_unc_scale.colToSave.append(final_weight_name)
            for var in vars_to_save:
                DefineBinnedColumn(dfw_unc_scale,  hist_cfg_dict, var, final_weight_name, unc, scale)
                dfw_unc_scale.colToSave.append(f"{var}_bin")
            final_treeName = f"{treeName}_{unc}_{scale}" if unc != 'Central' else treeName
            ...
            snaps.append(dfw_unc_scale.df.Snapshot(final_treeName, tmp_fileName, varToSave, snapshotOptions))
...
if __name__ == "__main__":
    ...
    if not dont_create_HistTuple :
        ...
        tmp_fileNames = createHistTuple(args.inFile, args.cacheFile, treeName, args.outFile, setup, hist_cfg_dict,unc_cfg_dict, snapshotOptions,args.nEvents, args.evtIds,  args.compute_unc_variations, args.compute_rel_weights, histTupleDef)
        ...
        if os.path.exists(args.outFile) and len(tmp_fileNames) !=0 :
            ...
            # if args.test : print(file_syst)
                if file_syst == args.outFile: continue
                os.remove(file_syst)
