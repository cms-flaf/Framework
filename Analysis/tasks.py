...
class Merge2DTaskCentral(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def workflow_requires(self):
        workflow_dict = {}
        workflow_dict["histProducerSample2D"] = {
            0: HistSample2DTaskCentral.req(self, branches=())
        }
        return workflow_dict

    def requires(self):
        deps = [HistSample2DTaskCentral.req(self, branches=()) ]
        return deps
    ...
    def output(self):
        var = self.branch_data
        outDir = os.path.join(self.central_Histograms_path(), 'all_histograms',var,self.GetBTagDir())
        outFile = f'all_histograms_{var}2D_onlyCentral.root'
        json_dir= os.path.join(self.valeos_path(), 'jsonFiles', self.period, self.version, 'all_ratios')
        json_file = f'all_ratios_{var}2D_onlyCentral.json'
        if not os.path.exists(outDir):
            os.makedirs(outDir)
        if not os.path.exists(json_dir):
            os.makedirs(json_dir)
...
class Plotter2DTaskCentral(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def output(self):
        ...
        plotdirname = 'plots_2p5' if self.version.split('_')[1] == 'deepTau2p5' else 'plots_2p1'
        eosdir = '/eos/home-v/vdamante/www/'
        final_directory = os.path.join(eosdir, plotdirname, var, self.GetBTagDir(), 'from2D')
        #final_directory = os.path.join(self.central_Histograms_path(), 'plots', var, self.GetBTagDir())
        final_fileName = f'{var}_{channel}_{category}_XMass{self.mass}.pdf'
        #final_fileName = f'{var}_{channel}_{QCD_region}_{category}_XMass{self.mass}.pdf'
        final_file= os.path.join(final_directory, final_fileName)
    ...
    def run(self):
        ...
        eosdir = '/eos/home-v/vdamante/www/'
        outDir = os.path.join(eosdir, plotdirname)
        PlotterProducer_cmd = ['python3', PlotterProducer,'--mass', self.mass, '--histDir', self.central_Histograms_path(), '--outDir',outDir, '--inFileName', 'all_histograms', '--var', var, '--sampleConfig',sample_config, '--channel', channel, '--category', category, '--uncSource', uncName, '--suffix','_onlyCentral', '--want2D' ,'True']
        sh_call(PlotterProducer_cmd,verbose=1)
    ...
    def create_branch_map(self):
        ...
        for var in vars_to_plot:
            for prod_br, (sample_id, sample_name, sample_type, input_file) in anaProd_branch_map.items():
                ...
                if ('QCD' in sample_name) or ('VBF' in sample_name and ('Radion' in sample_name or 'BulkGraviton' in sample_name)): continue
                branches[n] = (sample_name, prod_br,var)
                n+=1
            branches[n] = ('data', 0, var)
            n+=1
