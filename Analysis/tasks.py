...
class HistTupleProducerTask(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def workflow_requires(self):
        ...
        producer_set = set()
        for idx, (sample_name, br, need_cache_global, producer_list, input_index) in self.branch_map.items():
            branch_set.add(br)
    ...
    def create_branch_map(self):
        ...
        for prod_br,(sample_name, sample_type, input_file_list, output_file_list) in anaProd_branch_map.items():
            ...
            for input_index in range(len(output_file_list)):
                need_cache_global = any(need_cache_list) # If any of the variables needs cache, then we need to run the cache producer
                branches[n] = (sample_name, prod_br, need_cache_global, producer_list, input_index)
                n += 1
        return branches
    ...
    def run(self):
        ...
        HistTupleProducer = os.path.join(self.ana_path(), 'FLAF', 'Analysis', 'HistTupleProducer.py')
        ...
        for  producer_name in producer_list:
            ...
            with input_file.localize("r") as local_input:
                tmpFile = os.path.join(job_home, f'HistTupleProducerTask.root')
                HistTupleProducer_cmd = [ 'python3', HistTupleProducer,
                                        '--inFile', local_input.path, '--outFile', tmpFile,
                                        '--dataset', sample_name, '--uncConfig', unc_config, '--histTupleDef', histTupleDef,
                                        '--histConfig', self.setup.hist_config_path, '--period', self.period, '--channels', channels]
                if compute_unc_histograms:
                    HistTupleProducer_cmd.extend(['--compute_rel_weights', 'True', '--compute_unc_variations', 'True']) #should be split?
