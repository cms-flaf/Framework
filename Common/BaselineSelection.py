...
def RecoLeptonsSelection(df, apply_filter=True):
    ...
    """) # ((Muon_tightId || Muon_mediumId) && Muon_pfRelIso04_all < 0.5)
    df = df.Define("Tau_B0", f"""
        v_ops::pt(Tau_p4) > 15 && abs(v_ops::eta(Tau_p4)) < 2.3 && abs(Tau_dz) < 0.2 && Tau_decayMode != 5 && Tau_decayMode != 6
...
def RecoHttCandidateSelection(df, config):
    ...
    df = df.Define("Electron_B2_eTau_1", f"Electron_B0 && v_ops::pt(Electron_p4) > 20 ")
    ...
    df = df.Define("Muon_B2_muTau_1", f"""
        Muon_B0 && v_ops::pt(Muon_p4) > 20 && (   (Muon_tightId ) || (v_ops::pt(Muon_p4)>120 && Muon_highPtId ) )
    """) # --> removed for sync Muon_pfRelIso04_all < 0.15 Muon_tkRelIso < 0.15
