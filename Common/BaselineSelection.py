...
def RecoLeptonsSelection(df, config, apply_filter=True):
    df = df.Define("Electron_B0", f""" v_ops::pt(Electron_p4) > 10 && abs(v_ops::eta(Electron_p4)) < 2.5 && abs(Electron_dz) < 0.2 && abs(Electron_dxy) < 0.045  """) # && (Electron_mvaIso_WP90 || (Electron_mvaNoIso_WP90 && Electron_pfRelIso03_all < 0.5))

    df = df.Define("Muon_B0", f""" v_ops::pt(Muon_p4) > 15 && abs(v_ops::eta(Muon_p4)) < 2.4 && abs(Muon_dz) < 0.2 && abs(Muon_dxy) < 0.045 """) # && ( ((Muon_tightId || Muon_mediumId) && Muon_pfRelIso04_all < 0.5) || (Muon_highPtId && Muon_tkRelIso < 0.5) )

    eta_cut = 2.3 if config["deepTauVersion"] == '2p1' else 2.5

    df = df.Define("Tau_B0", f""" v_ops::pt(Tau_p4) > 20 && abs(v_ops::eta(Tau_p4)) < {eta_cut} && abs(Tau_dz) < 0.2 && Tau_decayMode != 5 && Tau_decayMode != 6 && ( Tau_idDeepTau{deepTauVersions[config["deepTauVersion"]]}v{config["deepTauVersion"]}VSjet >= {WorkingPointsTauVSjet.VVVLoose.value} ) """)

    df = df.Define("Electron_B0T", """  Electron_B0 && Electron_mvaIso_WP80 """)

    df = df.Define("Muon_B0T", """Muon_B0 && ( (Muon_tightId && Muon_pfRelIso04_all < 0.15) || (Muon_highPtId && Muon_tkRelIso < 0.15) ) """)

    df = df.Define("Tau_B0T", f"""
        Tau_B0 && ( Tau_idDeepTau2017v2p1VSjet >= {WorkingPointsTauVSjet.Loose.value} ) """)
    ...
    print(filter_expr)
    if apply_filter:
        print(f"before there were {df.Count().GetValue()} events")
        print(f"after there were {df.Filter(filter_expr).Count().GetValue()} events")
