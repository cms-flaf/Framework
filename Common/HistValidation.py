...
nums = ["jet_pt_eta_0_Loose","jet_pt_eta_0_Medium","jet_pt_eta_0_Tight","jet_pt_eta_4_Loose","jet_pt_eta_4_Medium","jet_pt_eta_4_Tight","jet_pt_eta_5_Loose","jet_pt_eta_5_Medium","jet_pt_eta_5_Tight",]
        ...
        for y_bin_number in range(1, hist.GetNbinsY()-1):
            ...
            if(bin_content==0):
                print(f""" key = {key},
                    ...
                    bin_center_y = {hist.GetYaxis().GetBinCenter(x_bin_number)}""")
prefix = "jet_pt_eta"
jet_flavors= [0, 4,5]
    ...
    for wp in wps:
        ...
        for x_bin_number in range(1, hist_den.GetNbinsX()-1):
            for y_bin_number in range(1, hist_den.GetNbinsY()-1):
                ...
                c_low = ROOT.TEfficiency.ClopperPearson(den, num, 1-0.68, False)
                c_up = ROOT.TEfficiency.ClopperPearson(den, num, 1-0.68, True)
                #print(f"low = {c_low}, up = {c_up}")
                eff= num/den
                rel_cl_size=(c_up- c_low)/(2*eff)
                if round(rel_cl_size,2)>0.01:
