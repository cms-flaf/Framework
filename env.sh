...
do_install_inference() {
    ...
    if ! [ -f "$this_dir/soft/CentOS$os_version/inference/.installed" ]; then
        run_cmd mkdir -p "$this_dir/soft/CentOS$os_version"
        run_cmd cd "$this_dir/soft/CentOS$os_version"
        # run_cmd source /cvmfs/sft.cern.ch/lcg/views/setupViews.sh LCG_102 x86_64-centos${os_version}-gcc11-opt
        run_cmd source /cvmfs/cms.cern.ch/cmsset_default.sh
        if [ -d inference ]; then
        ...
        run_cmd touch "$this_dir/soft/CentOS$os_version/inference/.installed"
    fi
}
...
install_cmssw() {
    ...
    if ! [ -f "$this_dir/soft/CentOS$os_version/$CMSSW_VER/.installed" ]; then
        run_cmd $env_cmd --command-to-run /usr/bin/env -i HOME=$HOME bash "$this_file" install_cmssw $scram_arch $cmssw_version $os_version $mode
    fi
}
...
install_inference() {
    ...
    if ! [ -f "$this_dir/soft/CentOS$os_version/inference/.installed" ]; then
        run_cmd $env_cmd --command-to-run /usr/bin/env -i HOME=$HOME bash "$this_file" install_inference $os_version
    fi
}
...
action() {
    ...
    alias cmbEnv="env -i HOME=$HOME ANALYSIS_PATH=$ANALYSIS_PATH ANALYSIS_DATA_PATH=$ANALYSIS_DATA_PATH X509_USER_PROXY=$X509_USER_PROXY CENTRAL_STORAGE=$CENTRAL_STORAGE ANALYSIS_BIG_DATA_PATH=$ANALYSIS_BIG_DATA_PATH DEFAULT_CMSSW_BASE=$CMB_CMSSW_BASE $ANALYSIS_PATH/RunKit/cmsEnv.sh"

    if [ $os_version -eq 7 ]; then
        alias infEnv="env -i HOME=$HOME ANALYSIS_PATH=$ANALYSIS_PATH ANALYSIS_DATA_PATH=$ANALYSIS_DATA_PATH X509_USER_PROXY=$X509_USER_PROXY CENTRAL_STORAGE=$CENTRAL_STORAGE ANALYSIS_BIG_DATA_PATH=$ANALYSIS_BIG_DATA_PATH INF_PATH=$ANALYSIS_PATH/soft/CentOS7/inference $ANALYSIS_PATH/inf_env.sh"
    else
        alias infEnv="env -i HOME=$HOME ANALYSIS_PATH=$ANALYSIS_PATH ANALYSIS_DATA_PATH=$ANALYSIS_DATA_PATH X509_USER_PROXY=$X509_USER_PROXY CENTRAL_STORAGE=$CENTRAL_STORAGE ANALYSIS_BIG_DATA_PATH=$ANALYSIS_BIG_DATA_PATH INF_PATH=$ANALYSIS_PATH/soft/CentOS7/inference /cvmfs/cms.cern.ch/common/cmssw-cc7 -- $ANALYSIS_PATH/inf_env.sh"
    fi
}
