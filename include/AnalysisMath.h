...
double Calculate_MT(const LVector1 &lepton_p4, const LVector2 &met_p4) {
  ...
  return std::sqrt(2.0 * lepton_p4.Pt() * met_p4.Pt() *
                   (1.0 - std::cos(delta_phi)));
}

template <typename LVector1, typename LVector2, typename LVector3>
double Calculate_TotalMT(const LVector1 &lepton1_p4, const LVector2 &lepton2_p4,
                         const LVector3 &met_p4) {
...
double Calculate_CosDTheta(const LVector1 &obj1_p4, const LVector2 &obj2_p4) {
  const ROOT::Math::DisplacementVector3D boost_vec =
      (obj1_p4 + obj2_p4).BoostToCM();
