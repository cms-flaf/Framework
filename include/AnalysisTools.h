...
template <typename... Legs> constexpr inline int LegsToInt(Legs... legs) {
...
inline LorentzVectorM GetP4(const RVecF &pt, const RVecF &eta, const RVecF &phi,
                            const RVecF &mass, int idx) {
...
RVecLV GetP4(const RVecF &pt, const RVecF &eta, const RVecF &phi,
             const RVecF &mass) {
  RVecLV p4;
  p4.reserve(pt.size());
...
RVecLV GetP4(const RVecF &pt, const RVecF &eta, const RVecF &phi,
             const RVecF &mass, const RVecS &indices) {
  RVecLV p4;
...
RVecB RemoveOverlaps(const RVecLV &obj_p4, const RVecB &pre_sel,
  ...
  for (size_t obj_idx = 0; obj_idx < obj_p4.size(); ++obj_idx) {
    result[obj_idx] =
        pre_sel[obj_idx] && hasMinNumberOfNonOverlaps(obj_p4.at(obj_idx));
  }
  return result;
}

RVecB RemoveOverlaps(const RVecLV &obj_p4, const RVecB &pre_sel,
                     const RVecLV &other_objects, double min_deltaR) {
  RVecB result(pre_sel);
  const double min_deltaR2 = std::pow(min_deltaR, 2);
...
int FindMatching(const LorentzVectorM &target_p4, const RVecLV &ref_p4,
                 const float deltaR_thr) {
  double deltaR_min = deltaR_thr;
  int current_idx = -1;
  for (int refIdx = 0; refIdx < ref_p4.size(); refIdx++) {
    auto dR_targetRef =
        ROOT::Math::VectorUtil::DeltaR(target_p4, ref_p4.at(refIdx));
...
RVecI FindMatching(const RVecLV &target_p4, const RVecLV &ref_p4,
                   const float deltaR_thr) {
...
int FindMatching(const bool pre_sel_target, const RVecB &pre_sel_ref,
                 const LorentzVectorM &target_p4, const RVecLV &ref_p4,
                 const float dR_thr) {
  ...
  if (pre_sel_target) {
    for (size_t ref_idx = 0; ref_idx < pre_sel_ref.size(); ref_idx++) {
      if (pre_sel_ref[ref_idx] == 0)
        continue;
      auto dR_targetRef =
          ROOT::Math::VectorUtil::DeltaR(target_p4, ref_p4[ref_idx]);
      if (dR_targetRef < deltaR_min) {
        current_idx = ref_idx;
        deltaR_min = dR_targetRef;
      }
...
RVecI FindMatching(const RVecB &pre_sel_target, const RVecB &pre_sel_ref,
                   const RVecLV &target_p4, const RVecLV &ref_p4,
                   const float deltaR_thr) {
  RVecI targetIndices(target_p4.size(), -1);
  for (int targetIdx = 0; targetIdx < target_p4.size(); targetIdx++) {
    targetIndices[targetIdx] =
        FindMatching(pre_sel_target[targetIdx], pre_sel_ref,
                     target_p4[targetIdx], ref_p4, deltaR_thr);
  }
  return targetIndices;
}

RVecSetInt FindMatchingSet(const RVecB &pre_sel_target,
                           const RVecB &pre_sel_ref, const RVecLV &target_p4,
                           const RVecLV &ref_p4, const float dR_thr) {
  RVecSetInt findMatching(pre_sel_target.size());
  for (size_t ref_idx = 0; ref_idx < pre_sel_ref.size(); ref_idx++) {
    if (pre_sel_ref[ref_idx] == 0)
      continue;
    for (size_t target_idx = 0; target_idx < pre_sel_target.size();
         target_idx++) {
      if (pre_sel_target[target_idx] == 0)
        continue;
      auto dR_current = ROOT::Math::VectorUtil::DeltaR(target_p4[target_idx],
                                                       ref_p4[ref_idx]);
...
namespace v_ops {
template <typename LV> RVecF pt(const LV &p4) {
  RVecF pt(p4.size());
...
template <typename LV> RVecF eta(const LV &p4) {
  RVecF eta(p4.size());
  for (int p4_idx = 0; p4_idx < p4.size(); ++p4_idx) {
...
template <typename LV> RVecF phi(const LV &p4) {
  RVecF phi(p4.size());
  for (int p4_idx = 0; p4_idx < p4.size(); ++p4_idx) {
...
template <typename LV> RVecF mass(const LV &p4) {
  RVecF m(p4.size());
  for (int p4_idx = 0; p4_idx < p4.size(); ++p4_idx) {
...
template <typename LV> RVecF energy(const LV &p4) {
  RVecF en(p4.size());
  for (int p4_idx = 0; p4_idx < p4.size(); ++p4_idx) {
...
template <typename LV> RVecF Et(const LV &p4) {
  RVecF et(p4.size());
  for (int p4_idx = 0; p4_idx < p4.size(); ++p4_idx) {
...
template <typename LV> RVecF rapidity(const LV &p4) {
...
ULong64_t encodeFullEventId(ULong64_t sample_name_crc, ULong64_t infile_crc,
                            ULong64_t rdfentry) {
...
std::tuple<ULong64_t, ULong64_t, ULong64_t>
decodeFullEventId(ULong64_t fullEventId) {
