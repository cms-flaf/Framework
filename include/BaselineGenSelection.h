...
void AssignHadronicVCand(VCand &cand, int cand_idx, const RVecI &GenPart_pdgId,
  ...
  std::sort(daughters.begin(), daughters.end(), [&GenPart_pt](int x, int y) {
    return GenPart_pt[x] > GenPart_pt[y];
  });

  const double cand_mass =
      ParticleDB::GetMass(GenPart_pdgId[cand_idx], GenPart_mass[cand_idx]);
  cand.cand_p4 =
      GetP4(GenPart_pt, GenPart_eta, GenPart_phi, cand_mass, cand_idx);
  ...
  for (size_t leg_id = 0; leg_id < cand.leg_p4.size(); ++leg_id) {
    ...
    const double mass =
        ParticleDB::GetMass(GenPart_pdgId[leg_idx], GenPart_mass[leg_idx]);
    cand.leg_p4[leg_id] =
        GetP4(GenPart_pt, GenPart_eta, GenPart_phi, mass, leg_idx);
...
void AssignLeptonicVCand(
    VCand &cand, int cand_idx,
    std::vector<reco_tau::gen_truth::GenLepton> const &gen_leptons,
    const RVecI &GenPart_pdgId, const RVecVecI &GenPart_daughters,
    const RVecI &GenPart_statusFlags, const RVecF &GenPart_pt,
    const RVecF &GenPart_eta, const RVecF &GenPart_phi,
    const RVecF &GenPart_mass) {
  cand.cand_p4 =
      GetP4(GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, cand_idx);
  ...
  for (size_t lep_id = 0; lep_id < cand.leg_p4.size(); ++lep_id) {
    ...
    if (PdG::isNeutrino(GenPart_pdgId[daughters[lep_id]])) {
      ...
      cand.leg_p4[lep_id] = GetP4(GenPart_pt, GenPart_eta, GenPart_phi,
                                  GenPart_mass, daughters[lep_id]);
      cand.leg_vis_p4[lep_id] = LorentzVectorM(0, 0, 0, 0);
    } else {
      reco_tau::gen_truth::GenLepton const *lep_ptr =
          findLeptonByIndex(gen_leptons, daughters[lep_id]);
      if (!lep_ptr)
...
VCand GetGenVCand(
    ...
    const RVecI &GenPart_statusFlags, const RVecF &GenPart_pt,
    const RVecF &GenPart_eta, const RVecF &GenPart_phi,
    const RVecF &GenPart_mass, RVecLV const &GenJet_p4) {
  VCand res;
  RVecI const &daughters = GenPart_daughters.at(v_idx);
  if (daughters.size() != 2) {
    throw analysis::exception(
        "Vector boson candidate has wrong number of daughters");
  }

  bool decays_to_hadrons =
      std::all_of(daughters.begin(), daughters.end(), [&](int idx) {
        return std::abs(GenPart_pdgId[idx]) <= PdG::b();
      });
  ...
  bool decays_to_leptons =
      std::all_of(daughters.begin(), daughters.end(), [&](int idx) {
        return PdG::isNeutrino(GenPart_pdgId[idx]) ||
               PdG::isChargedLepton(GenPart_pdgId[idx]);
      });
  if (decays_to_leptons) {
    AssignLeptonicVCand(res, v_idx, gen_leptons, GenPart_pdgId,
                        GenPart_daughters, GenPart_statusFlags, GenPart_pt,
                        GenPart_eta, GenPart_phi, GenPart_mass);
  }

  if (!decays_to_hadrons && !decays_to_leptons) {
    throw analysis::exception(
        "Encountered neither leptonic nor hadronic vector boson decay");
  }

  return res;
}

HVVCand GetGenHVVCandidate(
    int evt, std::vector<reco_tau::gen_truth::GenLepton> const &gen_leptons,
    const RVecI &GenPart_pdgId, const RVecVecI &GenPart_daughters,
    const RVecI &GenPart_statusFlags, const RVecF &GenPart_pt,
    const RVecF &GenPart_eta, const RVecF &GenPart_phi,
    const RVecF &GenPart_mass, RVecLV const &GenJet_p4,
  ...
  try {
    ...
    for (int i = 0; i < sz; ++i) {
      ...
      auto IsV = [&](int idx) {
        return GenPart_pdgId[idx] == PdG::Wplus() ||
               GenPart_pdgId[idx] == PdG::Wminus() ||
    ...
    HVV_cand.cand_p4 =
        GetP4(GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, HVV_index);

    const RVecI &V_from_H = GenPart_daughters.at(HVV_index);
    size_t n_leptons = 0;
    for (size_t v_id = 0; v_id < HVV_cand.legs.size(); ++v_id) {
      const int v_idx = GetLastCopy(V_from_H.at(v_id), GenPart_pdgId,
                                    GenPart_statusFlags, GenPart_daughters);
      HVV_cand.legs.at(v_id) =
          GetGenVCand(evt, v_idx, gen_leptons, GenPart_pdgId, GenPart_daughters,
                      GenPart_statusFlags, GenPart_pt, GenPart_eta, GenPart_phi,
                      GenPart_mass, GenJet_p4);
      HVV_cand.legs.at(v_id).index = v_idx;
      for (size_t leg_id = 0; leg_id < HVV_cand.legs.at(v_id).leg_p4.size();
           ++leg_id) {
    ...
    if (n_leptons != 2 && n_leptons != 4)
      throw analysis::exception("Encountered decay neither in single lepton "
    ...
    if (throw_error_if_not_found) {
      throw analysis::exception("GetGenHVVCandidate (event=%1%): %2%") % evt %
          e.message();
    }
    return {};
  }
...
HBBCand GetGenHBBCandidate(int evt, const RVecI &GenPart_pdgId,
                           const RVecVecI &GenPart_daughters,
                           const RVecI &GenPart_statusFlags,
                           const RVecF &GenPart_pt, const RVecF &GenPart_eta,
                           const RVecF &GenPart_phi, const RVecF &GenPart_mass,
                           ...
                           bool throw_error_if_not_found) {
  try {
    int const Hbb_index = GetGenHBBIndex(evt, GenPart_pdgId, GenPart_daughters,
                                         GenPart_statusFlags);
    HBBCand Hbb_cand;
    Hbb_cand.cand_p4 =
        GetP4(GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, Hbb_index);
    ...
    for (size_t leg_id = 0; leg_id < Hbb_cand.leg_p4.size(); ++leg_id) {
      ...
      Hbb_cand.leg_p4[leg_id] =
          GetP4(GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass, leg_idx);
      const int match = FindMatching(Hbb_cand.leg_p4[leg_id], GenJet_p4, 0.4);
      Hbb_cand.leg_vis_p4[leg_id] =
          match == -1 ? LorentzVectorM{} : GenJet_p4[match];
    }
    return Hbb_cand;
  } catch (analysis::exception &e) {
    if (throw_error_if_not_found) {
      throw analysis::exception("GetGenHBBCandidate (event=%1%): %2%") % evt %
          e.message();
    }
    return {};
  }
...
std::shared_ptr<HTTCand<2>>
GetGenHTTCandidate(int evt, const RVecI &GenPart_pdgId,
                   const RVecVecI &GenPart_daughters,
                   const RVecI &GenPart_statusFlags, const RVecF &GenPart_pt,
                   const RVecF &GenPart_eta, const RVecF &GenPart_phi,
                   const RVecF &GenPart_mass, bool throw_error_if_not_found) {
  try {
    ...
    for (int n = 0; n < GenPart_pdgId.size(); ++n) {
      ...
      int n_tau_daughters =
          std::count_if(daughters.begin(), daughters.end(), [&](int idx) {
      ...
      if (n_tau_daughters != 2)
        throw analysis::exception(
            "Invalid H->tautau decay. n_tau_daughters = %1%, higgs_idx = %2%") %
            n_tau_daughters % n;
    ...
    for (int tau_idx : GenPart_daughters.at(htt_index)) {
      if (std::abs(GenPart_pdgId.at(tau_idx)) != PdG::tau())
        continue;
      tau_idx = GetLastCopy(tau_idx, GenPart_pdgId, GenPart_statusFlags,
                            GenPart_daughters);
      ...
      for (int tau_daughter : GenPart_daughters.at(tau_idx)) {
        ...
        if (lepton_idx != tau_idx)
          throw analysis::exception("Invalid tau decay. tau_idx = %1%") %
              tau_idx;
        lepton_idx = tau_daughter;
      }

      if (!((lepton_idx == tau_idx && n_neutrinos == 1) ||
            (lepton_idx != tau_idx && n_neutrinos == 2)))
        throw analysis::exception(
            "Invalid number of neutrinos = %1% in tau decay. tau_idx = %2%") %
            n_neutrinos % tau_idx;

      lepton_idx = GetLastCopy(lepton_idx, GenPart_pdgId, GenPart_statusFlags,
                               GenPart_daughters);
    ...
    if (leg0_pdg > leg1_pdg ||
        (leg0_pdg == leg1_pdg && GenPart_pt.at(htt_cand.leg_index.at(0)) <
                                     GenPart_pt.at(htt_cand.leg_index.at(1))))
    ...
    for (leg_idx = 0; leg_idx < htt_cand.leg_index.size(); ++leg_idx) {
      ...
      htt_cand.leg_p4[leg_idx] =
          GetVisibleP4(genPart_index, GenPart_pdgId, GenPart_daughters,
                       GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass);
    }
    ...
    if (throw_error_if_not_found)
      throw analysis::exception("GetGenHTTCandidate (event=%1%): %2%") % evt %
          e.message();
...
int GetGenHBBIndex(int evt, const RVecI &GenPart_pdgId,
  ...
  try {
    ...
    for (int n = 0; n < GenPart_pdgId.size(); ++n) {
      ...
      if (n_b_daughters != 2)
        throw analysis::exception(
            "Invalid H->bb decay. n_b_daughters = %1%, higgs_idx = %2%") %
            n_b_daughters % n;
...
bool PassGenAcceptance(const HTTCand<2> &HTT_Cand) {
  for (size_t i = 0; i < HTT_Cand.leg_p4.size(); ++i) {
    if (!(HTT_Cand.leg_p4.at(i).pt() > 20 &&
          std::abs(HTT_Cand.leg_p4.at(i).eta()) < 2.3)) {
      return false;
    }
  }
  return true;
...
RVecB FindTwoJetsClosestToMPV(float mpv, const RVecLV &GenJet_p4,
                              const RVecB &pre_sel) {
  int i_min = -1, j_min = -1;
