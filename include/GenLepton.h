...
using LorentzVectorXYZ =
    ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double>>;
using LorentzVectorM =
...
public:
  ...
  static const PdgId PdgIdIntToEnum(int pdgId) {
    return static_cast<PdgId>(std::abs(pdgId));
  }

  static const std::set<PdgId> &gluonQuarks() {
    static const std::set<PdgId> s = {
        PdgId::gluon, PdgId::down,   PdgId::up,  PdgId::strange,
        PdgId::charm, PdgId::bottom, PdgId::top,
    };
    return s;
  }
  ...
  static const std::set<PdgId> &neutralLeptons() {
    static const std::set<PdgId> s = {
        PdgId::electron_neutrino, PdgId::muon_neutrino, PdgId::tau_neutrino};
    return s;
  }

  static const std::set<PdgId> &chargedHadrons() {
    static const std::set<PdgId> s = {PdgId::pi, PdgId::K, PdgId::K_star,
                                      PdgId::p,  PdgId::D, PdgId::D_s};
    return s;
  }

  static const std::set<PdgId> &neutralHadrons() {
    static const std::set<PdgId> s = {PdgId::pi0,   PdgId::K0_L, PdgId::K0_S,
                                      PdgId::K0,    PdgId::eta,  PdgId::omega,
                                      PdgId::J_psi, PdgId::D0};
    return s;
  }

  static const std::set<PdgId> &neutralBosons() {
    static const std::set<PdgId> s = {PdgId::photon, PdgId::Z, PdgId::W,
                                      PdgId::h0};
    return s;
  }
  ...
  static bool isQuarkGluon(PdgId pdgId) {
    return gluonQuarks().count(pdgId) > 0;
  }
  static bool isChargedLepton(PdgId pdgId) {
    return chargedLeptons().count(pdgId) > 0;
  }
  static bool isNeutralLepton(PdgId pdgId) {
    return neutralLeptons().count(pdgId) > 0;
  }
  static bool isChargedHadron(PdgId pdgId) {
    return chargedHadrons().count(pdgId) > 0;
  }
  static bool isNeutralHadron(PdgId pdgId) {
    return neutralHadrons().count(pdgId) > 0;
  }
  static bool isNeutralBoson(PdgId pdgId) {
    return neutralBosons().count(pdgId) > 0;
  }
  static bool isChargedBoson(PdgId pdgId) {
    return chargedBosons().count(pdgId) > 0;
  }
  static bool isKnownParticle(PdgId pdgId) {
    return knownParticles().count(pdgId) > 0;
  }
  static bool isLepton(PdgId pdgId) {
    return isChargedLepton(pdgId) || isNeutralLepton(pdgId);
  }
  static bool isHadron(PdgId pdgId) {
    return isChargedHadron(pdgId) || isNeutralHadron(pdgId);
  }
  static bool isBoson(PdgId pdgId) {
    return isChargedBoson(pdgId) || isNeutralBoson(pdgId);
  }
  ...
  static bool checkPdgId(PdgId pdgId, bool verbose) {
    ...
    if (unknownPdgIds.count(pdgId) == 0) {
      ...
      if (verbose)
        std::cerr << "GenLepton: unknown pdgId = " << static_cast<int>(pdgId)
                  << std::endl;
    }
    return false;
  }
...
inline std::ostream &operator<<(std::ostream &os, const GenParticle &p) {
  os << "index=" << p.index << " pdgId=" << p.pdgId << " pt=" << p.p4.pt()
     << " eta=" << p.p4.eta() << " phi=" << p.p4.phi() << " E=" << p.p4.energy()
     << " m=" << p.p4.mass() << " q=" << p.charge;
  if (p.vertex) {
    os << " vx=" << p.vertex->x() << " vy=" << p.vertex->y()
       << " vz=" << p.vertex->z() << " vrho=" << p.vertex->rho()
       << " vr=" << p.vertex->r();
  }
  return os;
}
...
public:
  ...
  static std::vector<GenLepton>
  fromGenParticleCollection(const std::vector<GenParticleT> &gen_particles) {
    ...
    for (const auto &particle : gen_particles) {
      if (processed_particles.count(&particle))
        continue;
      if (!(particle.statusFlags().isPrompt() &&
            particle.statusFlags().isFirstCopy()))
        continue;
  ...
  template <typename VectorPt, typename VectorEta, typename VectorPhi,
            typename VectorMass, typename VectorIdxMother, typename VectorPdgId,
            typename VectorStatusFlags, typename VectorVx, typename VectorVy,
            typename VectorVz>
  static std::vector<GenLepton>
  fromNanoAOD(const VectorPt &GenPart_pt, const VectorEta &GenPart_eta,
    ...
    using FillImpl =
        FillImplNano<VectorPt, VectorEta, VectorPhi, VectorMass,
                     VectorIdxMother, VectorPdgId, VectorStatusFlags, VectorVx,
                     VectorVy, VectorVz>;
    try {
      std::vector<GenLepton> genLeptons;
      std::set<size_t> processed_particles;
      for (size_t genPart_idx = 0; genPart_idx < GenPart_pt.size();
           ++genPart_idx) {
        if (processed_particles.count(genPart_idx))
          continue;
        GenStatusFlags particle_statusFlags(
            GenPart_statusFlags.at(genPart_idx));
        if (!(particle_statusFlags.isPrompt() &&
              particle_statusFlags.isFirstCopy()))
          continue;
        const auto pdg_code =
            GenParticle::PdgIdIntToEnum(GenPart_pdgId.at(genPart_idx));
        if (!GenParticle::isChargedLepton(pdg_code))
          continue;
        GenLepton lepton;
        FillImpl fillImplNano(
            lepton, processed_particles, GenPart_pt, GenPart_eta, GenPart_phi,
            GenPart_mass, GenPart_genPartIdxMother, GenPart_pdgId,
            GenPart_statusFlags, GenPart_vx, GenPart_vy, GenPart_vz);
  ...
  fromNanoAOD(const VectorPt &GenPart_pt, const VectorEta &GenPart_eta,
    ...
    return fromNanoAOD(GenPart_pt, GenPart_eta, GenPart_phi, GenPart_mass,
                       GenPart_genPartIdxMother, GenPart_pdgId,
                       GenPart_statusFlags, empty, empty, empty, event);
  }

  template <typename IntVector, typename LongVector, typename FloatVector>
  static GenLepton fromRootTuple(
      int lastMotherIndex, const IntVector &genParticle_pdgId,
      const LongVector &genParticle_mother, const IntVector &genParticle_charge,
      const IntVector &genParticle_isFirstCopy,
      const IntVector &genParticle_isLastCopy,
      const FloatVector &genParticle_pt, const FloatVector &genParticle_eta,
      const FloatVector &genParticle_phi, const FloatVector &genParticle_mass,
    ...
    try {
      ...
      for (size_t n = 0; n < N; ++n) {
        ...
        p.vertex = Point3D(genParticle_vtx_x.at(n), genParticle_vtx_y.at(n),
                           genParticle_vtx_z.at(n));
        std::set<size_t> mothers;
        Long64_t mother_encoded = genParticle_mother.at(n);
        if (mother_encoded >= 0) {
          do {
            Long64_t mother_index =
                mother_encoded % static_cast<Long64_t>(MaxNumberOfParticles);
            mother_encoded = (mother_encoded - mother_index) /
                             static_cast<int>(MaxNumberOfParticles);
  ...
  static const GenParticle *findTerminalCopy(const GenParticle &genParticle,
                                             bool first) {
    const GenParticle *particle = &genParticle;
    while ((first && !particle->isFirstCopy) ||
           (!first && !particle->isLastCopy)) {
  ...
  const std::set<const GenParticle *> &mothers() const {
    return firstCopy_->mothers;
  }
  ...
  const std::set<const GenParticle *> &finalStateFromDecay() const {
    return finalStateFromDecay_;
  }
  const std::set<const GenParticle *> &finalStateFromRadiation() const {
    return finalStateFromRadiation_;
  }
  const std::set<const GenParticle *> &hadrons() const { return hadrons_; }
  // Intermediate hadrons are hadrons that decayed hadronically
  const std::set<const GenParticle *> &intermediateHadrons() const {
    return intermediateHadrons_;
  }
  ...
  void PrintDecay(const GenParticle &particle, const std::string &pre,
                  std::ostream &os) const {
    os << particle << std::endl;

    for (auto d_iter = particle.daughters.begin();
         d_iter != particle.daughters.end(); ++d_iter) {
      const GenParticle &daughter = **d_iter;
      os << pre << "+-> ";
      const char pre_first =
          std::next(d_iter) == particle.daughters.end() ? ' ' : '|';
...
private:
  template <typename GenParticleT> struct FillImpl {
    ...
    FillImpl(GenLepton &lepton,
             std::map<const GenParticleT *, int> &processedParticles)
    ...
    void FillAll(const GenParticleT *particle) {
      ...
      if (last_mother_index != NoneIndex) {
        ...
        for (size_t mother_index = 0; mother_index <= last_mother_index;
             ++mother_index) {
          lepton_.particles_->at(last_mother_index + 1)
              .mothers.insert(&lepton_.particles_->at(mother_index));
          lepton_.particles_->at(mother_index)
              .daughters.insert(lepton_.firstCopy_);
        }
      } else {
      ...
      for (const auto &[mother, daughters] : relations_) {
        for (size_t daughter : daughters) {
          lepton_.particles_->at(mother).daughters.insert(
              &lepton_.particles_->at(daughter));
          lepton_.particles_->at(daughter).mothers.insert(
              &lepton_.particles_->at(mother));
        }
      }
    }

    void FillDaughters(const GenParticleT *p, size_t mother_index,
                       bool fill_recursively) {
      if (fill_recursively) {
  ...
  struct FillImplNano {
    ...
    FillImplNano(GenLepton &lepton, std::set<size_t> &processedParticles,
        ...
        : lepton_(lepton), processedParticles_(processedParticles),
          GenPart_pt_(GenPart_pt), GenPart_eta_(GenPart_eta),
          GenPart_phi_(GenPart_phi), GenPart_mass_(GenPart_mass),
          GenPart_genPartIdxMother_(GenPart_genPartIdxMother),
          GenPart_pdgId_(GenPart_pdgId),
          GenPart_statusFlags_(GenPart_statusFlags), GenPart_vx_(GenPart_vx),
          GenPart_vy_(GenPart_vy), GenPart_vz_(GenPart_vz) {}
    ...
    void FillDaughters(size_t part_idx, size_t mother_index,
                       bool fill_recursively) {
      if (fill_recursively) {
      ...
      const double output_pMass =
          GenParticle::getMass(output_p.pdgCode(), GenPart_mass_.at(part_idx));
      output_p.p4 =
          LorentzVectorM(GenPart_pt_.at(part_idx), GenPart_eta_.at(part_idx),
                         GenPart_phi_.at(part_idx), output_pMass);
      if (GenPart_vx_.size() == GenPart_pt_.size())
        output_p.vertex =
            Point3D(GenPart_vx_.at(part_idx), GenPart_vy_.at(part_idx),
                    GenPart_vz_.at(part_idx));
      ...
      if (fill_recursively) {
        ...
        for (size_t daughter_idx = part_idx + 1;
             daughter_idx < GenPart_pt_.size(); daughter_idx++) {
  ...
  void fillParticleCollections(const GenParticle &particle, bool fromLastCopy,
    ...
    if (particle.isLastCopy) {
      const bool isChargedHadron =
          GenParticle::isChargedHadron(particle.pdgCode());
      const bool isNeutralHadron =
          GenParticle::isNeutralHadron(particle.pdgCode());
      const bool isOther =
          !(isFinalState || isChargedHadron || isNeutralHadron);
      if (isFinalState) {
        auto &finalStateSet =
            fromLastCopy ? finalStateFromDecay_ : finalStateFromRadiation_;
...
private:
  ...
  std::set<const GenParticle *> finalStateFromDecay_, finalStateFromRadiation_,
      hadrons_, intermediateHadrons_, other_;
  LorentzVectorXYZ visibleP4_, radiatedP4_;
  size_t nChargedHadrons_{0}, nNeutralHadrons_{0}, nFinalStateElectrons_{0},
      nFinalStateMuons_{0}, nFinalStateNeutrinos_{0}, nFinalStatePhotons_{0};
};

inline std::ostream &operator<<(std::ostream &os, const GenLepton &lepton) {
  os << "kind=" << static_cast<int>(lepton.kind())
     << " nChargedHadrons=" << lepton.nChargedHadrons()
     << " nNeutralHadrons=" << lepton.nNeutralHadrons()
     << " nFinalStatePhotons=" << lepton.nFinalStatePhotons()
     << " vis_pt=" << lepton.visibleP4().pt()
     << " vis_eta=" << lepton.visibleP4().eta()
     << " vis_phi=" << lepton.visibleP4().phi()
     << " vis_E=" << lepton.visibleP4().energy()
     << " vis_m=" << lepton.visibleP4().mass()
     << " rad_pt=" << lepton.radiatedP4().pt()
     << " rad_eta=" << lepton.radiatedP4().eta()
     << " rad_phi=" << lepton.radiatedP4().phi()
     << " rad_E=" << lepton.radiatedP4().energy()
...
inline const GenLepton *findLeptonByIndex(const std::vector<GenLepton> &leptons,
                                          size_t index) {
