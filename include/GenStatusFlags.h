...
public:
  GenStatusFlags() {}
  GenStatusFlags(uint16_t flags_) : flags(flags_) {}
...
public:
  ...
  // is particle a decayed hadron, muon, or tau (does not include resonance
  // decays like W,Z,Higgs,top,etc) This flag is equivalent to status 2 in the
  // current HepMC standard but older generators (pythia6, herwig6) predate this
  // and use status 2 also for other intermediate particles/states
  ...
  // this particle is the direct descendant of a hard process particle of the
  // same pdg id
  ...
  bool isHardProcessTauDecayProduct() const {
    return flags[kIsHardProcessTauDecayProduct];
  }
  void setIsHardProcessTauDecayProduct(bool b) {
    flags[kIsHardProcessTauDecayProduct] = b;
  }

  // this particle is a direct decay product of a tau
  // from the hard process
  bool isDirectHardProcessTauDecayProduct() const {
    return flags[kIsDirectHardProcessTauDecayProduct];
  }
  void setIsDirectHardProcessTauDecayProduct(bool b) {
    flags[kIsDirectHardProcessTauDecayProduct] = b;
  }

  // this particle is the direct descendant of a hard process particle of the
  // same pdg id For outgoing particles the kinematics are those before QCD or
  // QED FSR This corresponds roughly to status code 3 in pythia 6
  bool fromHardProcessBeforeFSR() const {
    return flags[kFromHardProcessBeforeFSR];
  }
  void setFromHardProcessBeforeFSR(bool b) {
    flags[kFromHardProcessBeforeFSR] = b;
  }

  // this particle is the first copy of the particle in the chain with the same
  // pdg id
  ...
  const std::string toString() const {
    static const std::map<StatusBits, std::string> bitNames = {
        ...
        {StatusBits::kIsDirectPromptTauDecayProduct,
        ...
        {StatusBits::kIsHardProcessTauDecayProduct,
         "IsHardProcessTauDecayProduct"},
        {StatusBits::kIsDirectHardProcessTauDecayProduct,
