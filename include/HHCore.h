...
struct VCand {
  ...
  std::array<LorentzVectorM, n_legs>
      leg_vis_p4; // p4 of jet matched to quark; if no match = 0
...
template <size_t N> struct HTTCand {
  ...
  Channel channel() const {
    return _channel(std::make_index_sequence<n_legs>{});
  }

  bool operator==(const HTTCand &other) const {
    for (size_t idx = 0; idx < n_legs; ++idx) {
      if (leg_type[idx] != other.leg_type[idx] ||
          leg_index[idx] != other.leg_index[idx])
        return false;
    }
    return true;
  }
...
private:
  template <size_t... Idx> Channel _channel(std::index_sequence<Idx...>) const {
...
struct HWWCand {
  ...
  bool operator==(const HWWCand &other) const {
    for (size_t idx = 0; idx < n_legs; ++idx) {
      if (leg_type.at(idx) != other.leg_type.at(idx) ||
          leg_index.at(idx) != other.leg_index.at(idx))
        return false;
    }
    return true;
  }
