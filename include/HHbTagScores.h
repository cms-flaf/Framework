...
RVecF GetHHBtagScore(const RVecB &Jet_sel, const RVecI &Jet_idx,
                     const RVecLV &jet_p4, const RVecF &Jet_deepFlavour,
  ...
  auto hhBtag_period =
      PeriodToHHbTagInput(handle.version, static_cast<Period>(period));
  for (size_t jet_idx = 0; jet_idx < jet_p4.size(); jet_idx++) {
    ...
    rel_jet_M_pt.push_back(jet_p4.at(jet_idx_ordered).M() /
                           jet_p4.at(jet_idx_ordered).Pt());
    rel_jet_E_pt.push_back(jet_p4.at(jet_idx_ordered).E() /
                           jet_p4.at(jet_idx_ordered).Pt());
    jet_htt_deta.push_back(
        static_cast<float>(hTT_p4.Eta() - jet_p4.at(jet_idx_ordered).Eta()));
    jet_htt_dphi.push_back(
        ROOT::Math::VectorUtil::DeltaPhi(hTT_p4, jet_p4.at(jet_idx_ordered)));
  }

  RVecF goodJet_scores = handle.tagger->GetScore(
      jet_pt, jet_eta, rel_jet_M_pt, rel_jet_E_pt, jet_htt_deta,
      jet_deepFlavour, jet_htt_dphi, hhBtag_period, channelId, htt_pt, htt_eta,
      htt_met_dphi, rel_met_pt_htt_pt, htt_scalar_pt, parity);
