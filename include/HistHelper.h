...
namespace analysis {
typedef std::variant<int, float, bool, short, unsigned long, unsigned long long,
                     long long, long, unsigned int, RVecI, RVecF, RVecUC,
                     RVecUL, RVecULL, RVecSh, RVecB, double, unsigned char,
                     char>
...
struct Entry {
  ...
  template <typename T> void Add(int index, const T &value) {
  ...
  template <typename T> const T &GetValue(int idx) const {
...
static std::map<unsigned long long, std::shared_ptr<Entry>> &
GetCacheEntriesMap() {
  static std::map<unsigned long long, std::shared_ptr<Entry>> cache_entries;
  return cache_entries;
}

template <typename... Args> struct MapCreator {
                    ...
                    [&](const std::shared_ptr<Entry> &entry) {
                      ...
                      if (GetEntriesMap().find(idx) != GetEntriesMap().end()) {
                        ...
                        throw std::runtime_error(
                            "Duplicate cache_entry for index " +
                            std::to_string(idx));
                      }
...
template <typename... Args> struct CacheCreator {
        ...
        df_in
            ...
            .Define(map_name,
                    [&](const std::shared_ptr<Entry> &cache_entry) {
                      const auto idx =
                          cache_entry->GetValue<unsigned long long>(0);
                      if (GetCacheEntriesMap().find(idx) !=
                          GetCacheEntriesMap().end()) {
                        // if(checkDuplicates){
                        // std::cout << idx << "\t" << run << "\t" << evt <<
                        // "\t" << lumi << std::endl; throw
