...
std::shared_ptr<TFile> inline CreateRootFile(
  ...
  std::shared_ptr<TFile> file(
      TFile::Open(file_name.c_str(), "RECREATE", "",
                  compression * 100 + compression_level));
...
void WriteObject(const TObject &object, TDirectory *dir,
...
template <typename Object> void WriteObject(const Object &object) {
...
Object *ReadObject(TDirectory &file, const std::string &name) {
  ...
  if (!root_object)
    throw analysis::exception("Object '%1%' not found in '%2%'.") % name %
        file.GetName();
  Object *object = dynamic_cast<Object *>(root_object);
  if (!object)
    throw analysis::exception(
        "Wrong object type '%1%' for object '%2%' in '%3%'.") %
...
template <typename Object>
Object *CloneObject(const Object &original_object,
                    const std::string &new_name = "") {
  const std::string new_object_name =
      new_name.size() ? new_name : original_object.GetName();
  Object *new_object =
      dynamic_cast<Object *>(original_object.Clone(new_object_name.c_str()));
  if (!new_object)
    throw analysis::exception("Type error while cloning object '%1%'.") %
        original_object.GetName();
  return new_object;
}

template <typename Object>
Object *CloneObject(const Object &original_object, const std::string &new_name,
                    bool detach_from_file) {
...
template <typename Object>
Object *ReadCloneObject(TDirectory &file, const std::string &original_name,
                        const std::string &new_name = "",
...
TDirectory *GetDirectory(TDirectory &root_dir, const std::string &name,
  ...
  if (!dir && create_if_needed) {
    ...
    if (pos == std::string::npos || pos == name.size() - 1) {
      ...
      const std::string first_dir_name = name.substr(0, pos),
                        sub_dirs_path = name.substr(pos + 1);
  ...
  if (!dir)
    throw analysis::exception(
        "Unable to get directory '%1%' from the root directory '%2%'.") %
...
ClassInheritance FindClassInheritance(const std::string &class_name) {
  ...
  if (!cl)
    throw analysis::exception("Unable to get TClass for class named '%1%'.") %
        class_name;
  ...
  if (cl->InheritsFrom("TH1"))
    ...
    throw analysis::exception(
        "Unknown class inheritance for class named '%1%'.") %
        class_name;
