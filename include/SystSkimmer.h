...
namespace analysis {
typedef std::variant<int, float, bool, unsigned long, unsigned long long, long,
                     unsigned int, unsigned char, char, short, RVecI, RVecF,
                     RVecUC, RVecC, RVecS, RVecUL, RVecULL, RVecB>
    MultiType;
...
struct Entry {
  ...
  template <typename T> void Add(unsigned long long index, const T &value) {
    var_values.at(index) = value;
  }

  template <typename T> const T &GetValue(unsigned long long idx) const {
...
template <typename... Args> struct TupleMaker {
  ...
  void processIn(const std::vector<std::string> &var_names) {
    ...
    thread = std::make_unique<std::thread>([=]() {
      ...
      try {
        ...
        std::cout << "TupleMaker::processIn: exception: " << e.what()
                  << std::endl;
        throw;
      }
    ...
    df_out = df_out.Define(
        ...
        [=](ULong64_t FullEventIdShifted) {
          ...
          try {
            ...
            while (!entry || entry->GetValue<unsigned long long>(0) <
                                 FullEventIdShifted) {
              if (entry) {
            ...
            if (entry &&
                entry->GetValue<unsigned long long>(0) == FullEventIdShifted) {
              entryCentral = entry;
            }
          } catch (const std::exception &e) {
            std::cout << "TupleMaker::processOut: exception: " << e.what()
                      << std::endl;
            throw;
          }
          return entryCentral;
