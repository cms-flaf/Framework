...
namespace detail {
template <typename T, typename CharT> struct StringIOImpl {
...
template <typename CharT> struct StringIOImpl<std::basic_string<CharT>, CharT> {
  static std::basic_string<CharT>
  ToString(const std::basic_string<CharT> &str) {
    return str;
  }
  static bool TryParse(const std::basic_string<CharT> &str,
                       std::basic_string<CharT> &t, std::string &) {
    t = str;
    return true;
  }
};

template <typename CharT, int N> struct StringIOImpl<const CharT[N], CharT> {
...
std::basic_string<CharT> ToString(T &&t) {
  return detail::StringIOImpl<typename std::remove_reference<T>::type,
                              CharT>::ToString(std::forward<T>(t));
}
...
bool TryParse(const std::basic_string<CharT> &str, T &t,
              std::string &error_msg) {
  return detail::StringIOImpl<typename std::remove_reference<T>::type,
                              CharT>::TryParse(str, t, error_msg);
}
...
T Parse(const std::basic_string<CharT> &str) {
  ...
  if (!TryParse(str, t, error_msg)) {
    ...
    ss << "Parse of string \"" << str << "\" to " << typeid(T).name()
       << " is failed.";
    if (!error_msg.empty())
      ss << " " << error_msg;
...
std::string CollectionToString(const Collection &col,
...
inline std::vector<std::string> SplitValueList(const std::string &_values_str,
                                               bool allow_duplicates,
                                               const std::string &separators,
                                               bool enable_token_compress) {
  std::string values_str = _values_str;
  std::vector<std::string> result;
  auto is_any_of = [&](char c) {
  ...
  auto trim = [&](std::string &str) {
    ...
    for (; start_pos < str.size(); ++start_pos)
      if (!is_any_of(str[start_pos]))
        break;
    for (; end_pos > start_pos; --end_pos)
      if (!is_any_of(str[end_pos - 1]))
        break;
  ...
  if (!allow_duplicates) {
    ...
    for (const std::string &value : result) {
      if (set_result.count(value))
        throw exception(
            "Value '%1%' listed more than once in the value list '%2%'.") %
            value % values_str;
...
Collection SplitValueListT(const std::string &values_str,
                           bool allow_duplicates = true,
                           const std::string &separators = " \t",
                           bool enable_token_compress = true) {
  std::vector<std::string> list = SplitValueList(
      values_str, allow_duplicates, separators, enable_token_compress);
  Collection collection;
  std::transform(list.begin(), list.end(),
                 std::inserter(collection, collection.end()),
                 [](const std::string &str) { return Parse<T>(str); });
  return collection;
}

inline std::vector<std::string> ReadValueList(std::istream &stream,
  ...
  try {
    ...
    for (; n < number_of_items; ++n) {
      ...
      while (true) {
        ...
        if (predicate(c)) {
          if (!value.size() && enable_token_compress)
            continue;
          break;
        }
      ...
      if (!allow_duplicates && set_result.count(value))
        throw exception(
            "Value '%1%' listed more than once in the input stream.") %
            value;
      result.push_back(value);
      set_result.insert(value);
    }
    if (n != number_of_items)
      throw exception("Expected %1% items, while read only %2%.") %
          number_of_items % n;
...
namespace detail {
inline std::string LorentzVectorToString(double px, double py, double pz,
                                         double E, double mass, double pt,
                                         double eta, double phi,
                                         LVectorRepr repr, bool print_prefix) {
  ...
  if (print_prefix)
    ss << "(" << iter->second << ") = ";
  ss << "(";
  if (repr == LVectorRepr::PtEtaPhiM || repr == LVectorRepr::PtEtaPhiE ||
      repr == LVectorRepr::PtEtaPhiME) {
    ss << pt << ", " << eta << ", " << phi;
...
std::string LorentzVectorToString(const LVector &p4,
                                  LVectorRepr repr = LVectorRepr::PtEtaPhiME,
                                  bool print_prefix = true) {
  return detail::LorentzVectorToString(p4.Px(), p4.Py(), p4.Pz(), p4.E(),
                                       p4.M(), p4.Pt(), p4.Eta(), p4.Phi(),
                                       repr, print_prefix);
}
