...
template <typename T> struct DeltaImpl {
...
template <> struct DeltaImpl<bool> {
...
template <typename T> struct DeltaImpl<ROOT::VecOps::RVec<T>> {
  static ROOT::VecOps::RVec<T> Delta(const ROOT::VecOps::RVec<T> &shifted,
...
template <typename T> struct IsSameImpl {
  static bool IsSame(const T &shifted, const T &central) {
    return shifted == central;
  }
};

template <typename T> struct IsSameImpl<ROOT::VecOps::RVec<T>> {
  static bool IsSame(const ROOT::VecOps::RVec<T> &shifted,
                     const ROOT::VecOps::RVec<T> &central) {
...
template <typename T> T Delta(const T &shifted, const T &central) {
  return ::detail::DeltaImpl<T>::Delta(shifted, central);
}

template <typename T> T FromDelta(const T &delta, const T &central) {
  return ::detail::DeltaImpl<T>::FromDelta(delta, central);
}

template <typename T> bool IsSame(const T &shifted, const T &central) {
