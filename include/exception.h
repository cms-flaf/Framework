...
public:
  explicit exception(const std::string &message) noexcept
      : msg(std::make_unique<std::string>()),
        msg_valid(std::make_unique<bool>(false)), f_str(message) {
    try {
  ...
  exception(const exception &e) noexcept
      : msg(std::make_unique<std::string>(*e.msg)),
        msg_valid(std::make_unique<bool>(*e.msg_valid)), f_str(e.f_str),
  ...
  exception(exception &&e) noexcept
      : msg(std::move(e.msg)), msg_valid(std::move(e.msg_valid)),
        f_msg(std::move(e.f_msg)), f_str(e.f_str), stack_trace(e.stack_trace) {}
  ...
  const std::string &message() const noexcept {
    if (!*msg_valid) {
      try {
        ...
        *msg = "too few arguments are provided to the error message = '" +
               f_str + "'.";
  ...
  template <typename T> exception &operator%(const T &t) noexcept {
    try {
      if (!*msg_valid && f_msg)
        *f_msg % t;
    } catch (boost::io::too_many_args &) {
      *msg = "too many arguments are provided to the error message = '" +
             f_str + "'.";
      *msg_valid = true;
    } catch (std::exception &e) {
      process_unexpected_exception(e);
...
private:
  void process_unexpected_exception(const std::exception &e) const {
    *msg = "An exception has been raised while creating an error message. "
           "Error message = '" +
           f_str + "'. Exception message = '" + e.what() + "'.";
    *msg_valid = true;
  }
