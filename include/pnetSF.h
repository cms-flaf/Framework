...
getSFmap(float pT_, const std::string &period_, const int whichType) {
  ...
  if (whichType == 0) {
    ...
    if (period_ == "Run2_2016_HIPM") {
      ...
      if (pT_ < 300) {
        ...
        // scaleFactors["Loose"] = {1.00916880, 0.1431627, -0.1431627}; // LP
        // down
      } else if (pT_ >= 300 && pT_ < 400) {
        scaleFactors["Tight"] = {1., 0., 0.};
        scaleFactors["Medium"] = {1., 0., 0.};
...
float getSFPNet(float pT_, const std::string &period_, const std::string &scale,
                const std::string &WP, const int whichType) {
